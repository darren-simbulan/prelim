...
// ✅ I-check kung may `uploads/` Folder
$targetDir = "uploads/"; // Relative Path
if (!is_dir($targetDir)) {
    mkdir($targetDir, 0777, true); // Gumawa ng folder kung wala
}
...
if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
    ...
    if (move_uploaded_file($_FILES["image"]["tmp_name"], $targetFilePath)) {
        $image = $targetFilePath; // ✅ Save ang image path sa database
    } else {
        die("Error: Image upload failed. Check folder permissions.");
    }
}

// I-save ang recipe sa database
